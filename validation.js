 ...
 * 
 * Містить функції для перевірки коректності email, телефону та обов'язкових полів.
 * 
...
export const validateContact = (contact) => {
    ...
    if (!contact.name?.trim()) errors.push("Ім'я обов'язкове");

    // Базова перевірка email (має містити @ та крапку)
    if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(contact.email)) errors.push("Невірний формат email");

    // Перевірка формату телефону (до прикладу, нехай це буде: XXX-XX-XX)
    if (!/^\d{3}-\d{2}-\d{2}$/.test(contact.phone)) errors.push("Телефон у форматі XXX-XX-XX");
